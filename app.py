...
if eco_score >= 80:
    ...
    st.markdown("""
    **Сіздің жетістігіңіз:**
    * ✅ Тұтыну деңгейіңіз өте төмен. Бұл қоршаған ортаға тигізер пайдаңыз зор.
    * 💡 **Кеңес:** Осы деңгейді сақтап қалу үшін үйдегі құрылғыларды розеткадан суырып жүруді ұмытпаңыз.
    """)
elif 50 <= eco_score < 80:
    st.info(f"⚡ Жақсы нәтиже, бірақ үнемдеуге мүмкіндік бар. Рейтинг: {eco_score}/100")
    st.markdown("""
    **Үнемдеу бойынша ұсыныстар:**
    * 🛍️ **Пластиктен бас тартыңыз:** Дүкенге барғанда пластик пакеттердің орнына **мата шоппер (эко-сөмке)** ұстаңыз. Бұл жылына 300-ден астам пакеттің қоқысқа кетпеуін қамтамасыз етеді.
    * 💧 **Суды үнемдеңіз:** Тіс жуғанда немесе ыдыс сабындағанда суды жауып қойыңыз. Бұл айына 500-1000 литр суды сақтап қалады.
    """)
else:
    st.warning(f"⚠️ Шығын өте көп! Ресурстарды шұғыл үнемдеу қажет. Рейтинг: {eco_score}/100")
    st.markdown(f"""
    **Шұғыл әрекет ету жоспары:**
    * 💡 **LED шамдарға көшіңіз:** Кәдімгі шамдарды **LED (жарық диодты)** шамдарға ауыстырыңыз. Олар 10 есе аз энергия жұмсайды және төлеміңізді 30%-ға дейін азайтады.
    * 🚿 **Душ қабылдау:** Ваннаға су толтырғаннан қарағанда, 5 минуттық душ қабылдау суды 3 есе аз жұмсайды.
    """)
